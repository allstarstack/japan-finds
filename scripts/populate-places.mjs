...
/* ── V1 migration table ────────────────────────────────────────────────
   The 17 Phase A places kept as genuine attractions. Keyed by their
   existing filename slug. primary_category / public_label / planning_flags
   are migration assignments — confirmed correct by Steven. */
const V1_KEEP = {
  "art-aquarium-ginza": {
...
/* The 18 V1 places dropped. 16 are retail/lodging — not destinations,
   already in the `stores` collection or a hotel brand. The last 2 are
   combined multi-place V1 entries superseded by the import's specific
   single-place rows ("prefer single-place over multi-place" — Steven). */
const V1_DROP = {
  ...
  "kanazawa-omicho-kenrokuen":
    "combined V1 entry — superseded by the import row omicho-market.yaml " +
    "(Ishikawa). FOLLOW-UP: add a separate Kenrokuen Garden entry via Quick-Add.",
  "beppu-yufuin-onsen-towns":
    "combined V1 entry — superseded by the import row yufuin-onsen-town.yaml " +
    "(Oita). FOLLOW-UP: add a separate Beppu Onsen entry via Quick-Add.",
};
...
L.push(`## V1 migration — ${v1Planned.length} attractions kept`, "");
L.push("Field assignments below — confirmed correct by Steven.", "");
L.push("| place | primary_category | chip | planning_flags |", "|---|---|---|---|");
for (const [slug, t] of Object.entries(V1_KEEP))
  L.push(`| ${t.name} | ${t.primary_category} | ${t.public_label || "—"} | ${t.planning_flags || "—"} |`);

L.push("", `## V1 dropped — ${Object.keys(V1_DROP).length} entries`, "");
L.push("Retail/lodging (in the `stores` collection already) or combined " +
  "multi-place entries superseded by specific import rows.", "");
...
console.log(`  dropped ${Object.keys(V1_DROP).length} V1 entries (retail/lodging + superseded)`);
